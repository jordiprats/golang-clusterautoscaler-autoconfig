...
_, err = clientset.CoreV1().ConfigMaps(caNamespace).Get(caPriorityExpander, metav1.GetOptions{})
if err == nil {
...
if !configMapExists {
	if skipCMCreation {
		...
		_, err := clientset.CoreV1().ConfigMaps(caNamespace).Create(&corev1.ConfigMap{
	...
	cm, err := clientset.CoreV1().ConfigMaps(caNamespace).Get(caPriorityExpander, metav1.GetOptions{})
	if err != nil {
		fmt.Printf("Error retrieving configmap: %v\n", err)
		return
	}
	cm.Data = data
	_, err = clientset.CoreV1().ConfigMaps(caNamespace).Update(cm)
	if err != nil {
